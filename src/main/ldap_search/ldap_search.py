...
def ldap_search(server_EnvVariable, user_name_EnvVariable, user_pass_EnvVariable, search_base_EnvVariable):
    ...
    # Search OU
    conn.search(search_base=search_base_EnvVariable, search_filter='(objectclass=computer)', attributes=['cn', 'distinguishedName', 'dNSHostName', 'lastLogon', 'operatingSystem', 'operatingSystemServicePack', 'operatingSystemVersion'])
    return conn.entries
